...
cd /github/workspace
cirun env CC=$(which gcc) cmake -S cmake.deps -B .deps -G Ninja -D USE_BUNDLED_LUAJIT=OFF -D USE_BUNDLED_LUA=ON || ( cat .deps/CMakeFiles/CMake*.log; false )
cirun cmake --build .deps
cirun env CC=$(which gcc) cmake -B build -G Ninja -D PREFER_LUA=ON || ( cat build/CMakeFiles/CMake*.log; false )
cirun make ${TARGET}
